...
public @interface TaskPriority {
   ...
   * CreateUpdateableSequencedTaskRunnerWithTraits() to increase the priority when that user
   * interactions happens).
   */
  int BEST_EFFORT = 0;
  /**
   * This task affects UI but it is not an immediate response to a user interaction. Examples: -
   * Updating the UI to reflect progress on a long task. - Loading an image that is displayed in the
   * UI but is non-critical.
   */
  int USER_VISIBLE = 1;
  /**
