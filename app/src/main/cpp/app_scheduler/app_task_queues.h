...
#ifndef __APP_TASK_QUEUES_H__
#define __APP_TASK_QUEUES_H__
...
// for them.
class BASE_EXPORT AppTaskQueues {
public:
  ...
  static constexpr size_t kNumQueueTypes = static_cast<size_t>(QueueType::kMaxValue);
  ...
  class BASE_EXPORT Handle {
   public:
    ...
    const scoped_refptr<base::SingleThreadTaskRunner>& GetAppTaskRunner(
        QueueType queue_type) const {
      return app_task_runners_[static_cast<size_t>(queue_type)];
    }
   ...
   private:
    ...
    friend class AppTaskQueues;
    explicit Handle(AppTaskQueues* task_queues);

    // |outer_| can only be safely used from a task posted to one of the
    // runners.
    AppTaskQueues* outer_ = nullptr;
 ...
 private:
  ...
  base::sequence_manager::TaskQueue* GetAppTaskQueue(QueueType type) const {
    return app_queues_and_voters_[static_cast<size_t>(type)].first.get();
  }

  std::array<scoped_refptr<base::SingleThreadTaskRunner>, kNumQueueTypes>
  CreateAppTaskRunners() const;

  using QueueVoterPair = std::pair<
      scoped_refptr<base::sequence_manager::TaskQueue>,
      std::unique_ptr<base::sequence_manager::TaskQueue::QueueEnabledVoter>>;

  std::array<QueueVoterPair, kNumQueueTypes> app_queues_and_voters_;
